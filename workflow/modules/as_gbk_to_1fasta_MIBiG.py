    ...
    try:
        for fp in file_paths:
            ...
            for record in records:
                ...
                sample_parts = record.description.split(" ") # sample name; note that if it is name_TR there can be problems          
                name=re.sub(r"[\[\]]","",sample_parts[0]) #delete [] around 
                #else:
                #for feat in record.features:
                    #if feat.type == "protocluster":
                    #    product = feat.qualifiers["product"][0] # Product is the expected output product
                    #    break
                fasta_entry=f">{name}_{contig}\n{record.seq}\n"
...
def get_files_path(parent_directory):
    AS_directories =glob.glob(os.path.join(parent_directory, "MIBiG")) # Find all directories that start with "AS_" in the directory we provide
    gbk_file_paths = []
