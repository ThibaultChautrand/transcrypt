...
def gbk_files_to_fasta(file_paths: List[str], output_file = "AS/ASclusters.fasta"):  # The file paths are going to be strings; the output file can be changed when calling the function, it is also a string
    out_fh = open(output_file, "w")
    out_table = open(output_file+".tsv","w")
    try:
        ID_dict={}
        counter=0
        for fp in file_paths:
            ...
            for record in records:
                ...
                sample_parts = record.description.split("__") # sample name; note that if it is name_TR there can be problems
                ...
                for feat in record.features:
                    if feat.type == "protocluster":
                        ...
                        break
                identifier="ID"+"{:05d}".format(counter)
                information=f"{name}\t{contig}\t{product}"
                fasta_entry=f">{identifier}__{product}\n{record.seq}\n"
                ...
                out_table.write(identifier+"\t"+information+"\n")
...
def get_files_path(parent_directory):
    ...
    for dir_path in AS_directories:
        for file_name in os.listdir(dir_path): # In every file from the directory finds those with extension .gbk
            if re.search("region[\d]*.gbk$", file_name): # to get only the as output files
