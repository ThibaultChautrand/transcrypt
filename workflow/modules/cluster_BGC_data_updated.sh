...
python as_gbk_to_1fasta_updated.py -i ${fasta_dir} -o ${fasta_file}
sed 's/> />/' ${fasta_dir}/${fasta_file} > ${fasta_dir}/clusters.fasta.temp
rm AS/ASclusters.fasta
mv AS/clusters.fasta.temp AS/ASclusters.fasta
